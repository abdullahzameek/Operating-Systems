...
global VERBOSE, SHOW_RANDOM, OPEN, randomFile
VERBOSE = False
SHOW_RANDOM = False
OPEN = True
...
class Process:  
    ...
    def updateState(self):
        ...
        if self.state == "running":
            if self.remainingTime == 0:
                ...
                self.turnaroundTime = self.finishTime - self.A
                self.burst = 0
            elif self.burst == 0:
                self.setBlocked()
        return
    ...
    def getRatio(self):
        return self.turnaroundTime/max(1,sysClock.getTime())

    def updateTimes(self):
        if self.state != "unstarted" or self.state != "terminated": #Nothing to do in these two cases 
...
def schedulingAlgorithm(processList, schedulingMethod):
    i = 0
